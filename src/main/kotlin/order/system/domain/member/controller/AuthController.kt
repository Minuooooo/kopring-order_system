...
import order.system.domain.member.dto.sign.TokenRequestDto
...
class AuthController(
    ...
    @Operation(summary = "Get naver email API", description = "please sign in naver.")
    @ResponseStatus(OK)
    ...
    fun signIn(@Valid @RequestBody signInRequestDto: SignInRequestDto): Response =
            success(SuccessMessage.SUCCESS_TO_SIGN_IN, authService.signIn(signInRequestDto))
    ...
    fun signInWithKakao(@RequestBody kakaoLoginParams: KakaoLoginParams): Response =
            success(SuccessMessage.SUCCESS_TO_SIGN_IN, authService.signInWithSocial(kakaoLoginParams))
    ...
    fun signInWithNaver(@RequestBody naverLoginParams: NaverLoginParams): Response =
            success(SuccessMessage.SUCCESS_TO_SIGN_IN, authService.signInWithSocial(naverLoginParams))
    ...
    @Operation(summary = "Reissue API", description = "put your token info.")
    @ResponseStatus(OK)
    @PostMapping("/reissue")
    fun reissue(@RequestBody tokenRequestDto: TokenRequestDto): Response =
            success(SuccessMessage.SUCCESS_TO_REISSUE, authService.reissue(tokenRequestDto))
}